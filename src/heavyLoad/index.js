...
import ITPAnimation from "./ITPAnimation";
import ITPAnimatedAPI from "./ITPAnimatedAPI";
...
export default class App extends React.Component {
  ...
  startTimer = () => {
    ...
    this.interval = setInterval(() => {
      const calculatedValue =
        this.state.value >= 1
          ? 0
          : Math.round((this.state.value + 0.01) * 1000) / 1000;
  ...
  render() {
    const AnimationComponent = this.state.useAnimationAPI
      ? ITPAnimatedAPI
      : ITPAnimation;
    const value = this.state.useAnimationAPI
      ? this.animatedValue
      : this.state.value;
    const rotation = this.state.useAnimationAPI
    ...
    const row = this.state.useNativeDriver ? (
      <View style={styles.row}>
        <AnimationComponent
          value={this.state.value}
          rotation={rotation}
          backgroundColor1={COLOR1(0.5)}
          backgroundColor1={COLOR2(0.3)}
          fontSize={30}
        />
        <AnimationComponent
          value={this.state.value}
